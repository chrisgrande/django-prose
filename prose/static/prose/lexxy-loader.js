(function () {
  function resolveLexxyModuleUrl() {
    var editor = document.querySelector("lexxy-editor.django-prose-lexxy[data-lexxy-module-url]");
    if (editor) {
      var fromEditor =
  ...
  function filePillFigureHtml(payload) {
    ...
    var sgidAttr = payload.sgid ? ' data-prose-sgid="' + escapeHtml(payload.sgid) + '"' : "";
    return (
  ...
  function proseAttachmentHtml(payload) {
    ...
    if (payload.kind === "embed") {
      ...
      if (payload.filename && (payload.content_type || "") !== YOUTUBE_CONTENT_TYPE) {
  ...
  function wireLinkEmbedButton(toolbar, editorElement, embedHost, checkHost, attempt) {
    attempt = attempt || 0;
    if (!toolbar) {
      toolbar = editorElement.toolbarElement || editorElement.querySelector("lexxy-toolbar");
    }
    if (!toolbar) {
      if (attempt < 30) {
        requestAnimationFrame(function () {
  ...
  function uploadFileToEditor(editorElement, host, file, pending) {
    ...
    uploadFile(
      ...
      function (err) {
        if (pending && typeof pending.remove === "function") pending.remove();
        showEditorUploadError(editorElement, (err && err.message) || "Could not upload this file.");
  ...
  function applyLexxyContentScope(editorElement) {
    var content =
      editorElement.editorContentElement || editorElement.querySelector(".lexxy-editor__content");
  ...
  var FILE_PILL_NAME_INPUT_SELECTOR = "figure.django-prose-file-pill .django-prose-file-pill__name";
  ...
  function purgeAttachmentFromEditorValue(editorElement, sgid) {
    ...
    next = next.replace(
      new RegExp("<prose-attachment\\b[^>]*\\ssgid=[\"']" + escaped + "[\"'][^>]*/?>", "gi"),
      "",
    );
    next = next.replace(
      new RegExp(
  ...
  function removeProseAttachmentFrom(node) {
    ...
    function finishRemove() {
      var valueHasSgid = sgid && editorElement && (editorElement.value || "").indexOf(sgid) !== -1;
  ...
  function bindFilePillControls(root, captionHost) {
    ...
    root
      .querySelectorAll("figure.django-prose-file-pill .django-prose-file-pill__remove")
      .forEach(bindFilePillRemoveButton);
  }
  ...
  function isolateFilePillsFromLexxy(editorElement) {
    ...
    editorElement.addEventListener(
      ...
      function (event) {
        if (
          isFilePillNameInput(event.target) ||
          event.target.closest(".django-prose-file-pill__remove")
        ) {
          event.stopPropagation();
        }
      },
